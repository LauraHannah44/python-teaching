def database_read():
    ...
    for user_string in user_strings:  # iterating through each user's string
        ...
        for i, info in enumerate(user_info_list):  # each piece of user info for a single user
            if key_list[i] in ("attempts"):  # dealing with ints
                user_dict[key_list[i]] = int(info)  # creating a new dictionary entry with the corresponding key
            else:
